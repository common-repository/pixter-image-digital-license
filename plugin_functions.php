...
function p1xtr_pixter_image_digital_license_plugin_loaded($pluginName){
    if (is_admin()) {
...
function p1xtr_pixter_image_digital_license_activate($pluginName){
    global $p1xtr_pixter_image_digital_license_fields;
    $default = array();
    foreach ($p1xtr_pixter_image_digital_license_fields as $conf)
...
function p1xtr_pixter_image_digital_license_activation_redirect($plugin, $pluginName)
{
    if ($plugin == plugin_basename(__FILE__))
        exit(wp_redirect(admin_url('admin.php?page='.$pluginName.'_plugin')));
}

function p1xtr_pixter_image_digital_license_show_plugin($pluginName)
{
    global $post;
...
function p1xtr_pixter_image_digital_license_inline_script($pluginName){
    $apiKey = get_option( $pluginName . '_user');
    if (!empty($apiKey) && !wp_script_is($pluginName.'_inline', 'done') && p1xtr_pixter_image_digital_license_show_plugin($pluginName)) {
        $options = (object)get_option($pluginName.'_options');
        ...
        if(!function_exists('getBoolString')){
            function getBoolString($inputArrayContainingEnabled){
                if($inputArrayContainingEnabled || (isset($inputArrayContainingEnabled['enabled']) && $inputArrayContainingEnabled['enabled'] == true)){
                    return 'true';
                }
                return 'false';
            }
...
function p1xtr_pixter_image_digital_license_register_notice($pluginName, $pluginDisplayName){
...
function p1xtr_pixter_image_digital_license_psk_notice($pluginName, $pluginDisplayName){
    $options = (object)get_option($pluginName.'_options');
    $toggle_psk = $options->toggle_psk;
    global $pixter_image_digital_license_admin_tools;
    if ($pixter_image_digital_license_admin_tools->getToggleBoolValue($toggle_psk) === false) {
